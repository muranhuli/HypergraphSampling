...
class RandomWalkSamplerWithMH(Sampler):
    ...
    def _do_a_step(self, graph):
        ...
        if weight_sum <1:
            vertices.append(-1)
            weight.append(1-weight_sum)
        tmp = random.choices(vertices,weights = weight,k =1)[0]
        
        # 没有选择到顶点直接退出
        if tmp == -1:
            return
        
        self._current_node = tmp
        ...
        # 没有选择到超边直接退出
        tmp = random.choices(edges,weights = weight,k =1)[0]
        if tmp == -1:
            return
        self._current_edge = tmp
        self._sample_data.sample_edge(self._current_edge)
        self._sample_data.sample_unique_edge(self._current_edge)
